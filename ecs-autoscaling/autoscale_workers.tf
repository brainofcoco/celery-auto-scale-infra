...
resource "aws_appautoscaling_target" "medium_worker_autoscaling_target" {
  ...
  resource_id        = "service/${var.ecs_cluster["name"]}/${var.ecs_cluster["service_name"]}"
...
resource "aws_appautoscaling_policy" "medium_worker_scale_up" {
  ...
  step_scaling_policy_configuration {
    ...
    step_adjustment {
      # metric_interval_lower_bound - The lower bound for the difference between the alarm threshold and the CloudWatch metric. Without a value, AWS will treat this bound as negative infinity.
      metric_interval_lower_bound = 30
      # metric_interval_upper_bound - The upper bound for the difference between the alarm threshold and the CloudWatch metric. Without a value, AWS will treat this bound as infinity. The upper bound must be greater than the lower bound.
      metric_interval_upper_bound = 50
      # scaling_adjustment - The number of members by which to scale, when the adjustment bounds are breached. A positive value scales up. A negative value scales down.
